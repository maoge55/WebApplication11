...
{
    public partial class 翻译成印尼语 : System.Web.UI.Page
    {
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            DataSet dsall = access_sql.GreatDs("select itemid from ProShopeePh where datatype=" + datatype + where + " and (yntitle is null or yntitle ='') group by itemid ");
            string count = "0";
            if (access_sql.yzTable(dsall))
            {
                count = dsall.Tables[0].Rows.Count.ToString();
                DataSet ds = access_sql.GreatDs("select itemid+'|'+pname as jg from ProShopeePh where itemid in(select top 100 itemid from ProShopeePh where datatype=" + datatype + where + " and (yntitle is null or yntitle ='') group by itemid) group by itemid,pname");
                    ...
                    foreach (string item in fz)
                    {
                        if (item != "")
                        {
                            ...
                            if (btfz.Length == 2)
                            {
                                ...
                                string yntitle = btfz[1].Trim().Replace("'", "''");
                                if (itemid != "" && yntitle != "")
                                {
                                    int cc = access_sql.T_Update_ExecSql(new string[] { "yntitle", "yntitleRcode" }, new object[] { yntitle, yntitle + " |" + code + "|" }, "ProShopeePh", "itemid='" + itemid + "'");
                                    if (cc > 0)
                                    {
                            ...
                            else
                            {
                                ...
                                yntitle = yntitle.Replace("'", "''");
                                if (yntitle != "")
                                {
                                    yntitle = yntitle.Substring(0, yntitle.Length - 1);
                                }
                                if (itemid != "" && yntitle != "")
                                {
                                    int cc = access_sql.T_Update_ExecSql(new string[] { "yntitle", "yntitleRcode" }, new object[] { yntitle, yntitle + " |" + code + "|" }, "ProShopeePh", "itemid='" + itemid + "'");
